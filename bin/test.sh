...
id=$(docker run -v `pwd`:/tmp/jenkins-pipeline-library --rm -d --privileged -p 8080:8080 redpandaci/jenkins-dind)
echo " with id ${id}"
...
docker exec ${id} wget http://localhost:8080/jnlpJars/jenkins-cli.jar > /dev/null
