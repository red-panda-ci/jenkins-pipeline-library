...
def downloadScripts (cfg) {
    if (cfg.flags.wereScriptsDownloaded) {
        ...
        sh "rm -rf ci-scripts/.jpl-scripts && mkdir -p ci-scripts/.temp && cd ci-scripts/.temp/ && wget -q -O - https://github.com/red-panda-ci/jpl-scripts/archive/develop.zip | jar xvf /dev/stdin > /dev/null && chmod +x ci-scripts-develop/bin/*.sh && mv ci-scripts-develop ../.jpl-scripts"
